...
class RoomManager {
    ...
    // Timer Management
    startTimer(room, durationSeconds, callback) {
        this.stopTimer(room.code);
    ...
    areNightActionsComplete(room) {
        ...
        if (mafiaVotedCount < aliveMafia.length)
            return false;
        // 2. Check Doctor
        const aliveDoctor = alivePlayers.filter(p => p.role === 'doctor');
        if (aliveDoctor.length > 0 && room.nightActions.doctorHeal === null)
            return false;
        // 3. Check Detective
        const aliveDetective = alivePlayers.filter(p => p.role === 'detective');
        if (aliveDetective.length > 0 && room.nightActions.detectiveInvestigate === null)
            return false;
        return true;
    }
